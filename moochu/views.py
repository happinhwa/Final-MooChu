...
def genre_filter(request, ott, media_type):
    ...
    if ott=='All':
        data = list(Media.collection.find({"genres": {"$elemMatch": {"$in": selected_genres}}, "mediaType":media_type}))
    else:
        data = list(Media.collection.find({"genres": {"$elemMatch": {"$in": selected_genres}}, "mediaType":media_type, 'OTT':ott }))
